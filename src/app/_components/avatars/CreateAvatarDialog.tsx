...
export function CreateAvatarDialog({ close }: {close: () => void }) {
    ...
    return         <Dialog >
    <ActionButton onPress={() => close() } isDisabled={mutation.isPending}>閉じる</ActionButton>
        <Heading>アバター作成</Heading>
        <Divider/>
        <Content>
            <Form onSubmit={onsubmit} validationBehavior="native" validationErrors={errors}>
                <TextField name="name" label="名前" description="アバターの名前（例: 山田太郎）" value={formdata.name} onChange={createOnFormChange("name")} isRequired/>
                <Button type="submit" variant="primary" isPending={mutation.isPending}>作成</Button>
                <Button type="reset" variant="secondary">リセット</Button>
            </Form>
        </Content>
    </Dialog>
}
