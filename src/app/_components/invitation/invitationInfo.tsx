...
export function InvitationInfo() {
    ...
    return <View>
        ...
        <RelationshipStatusActions isOwned={result.isOwned} relationshipStatus={result.relationshipStatus}/>
    </View>
}

// RelationshipStatusによって表示を出し分ける
function RelationshipStatusActions({isOwned, relationshipStatus }: {isOwned: boolean, relationshipStatus: RelationshipStatus}) {
    if(isOwned){
        return <View>
            <Text>これはあなたのアバターです。このURLを友達に教えることで、友達申請してもらうことができます。</Text>
        </View>
    }
    switch(relationshipStatus){
        ...
        case "REQUEST_RECEIVED":
            return <View>
                <Text>このアバターのユーザーからはすでに友達申請を受け取っています。</Text>
                <ActionButton>詳細を確認</ActionButton>
            </View>
